module CodegenElisp(codegenElisp) where
...
elispName :: Name -> String
elispName n = "idris_" ++ concatMap phpchar (showCG n)
...
cgBody ret (SV (Glob n)) = ret $ elispName n ++ "()"
cgBody ret (SV (Loc i)) = ret $ loc i
cgBody ret (SApp _ f args) = ret $ elispName f ++ "(" ++
